...
//-------------------------------seting pwm  --------
  ...
  if (type == 0){
   io= (ios*512);  // PWM freq 128 256 1024 .. 4096
  // element= (element*ios);
  }
  else{
  io = 512;
  ...
  if (GTItype == 0){
  //  GTIios = (16 - ios);
    GTIio = (GTIios*512); // PWM freq 128 256 1024 .. 4096
  }
  else{
    GTIio=512;
}
//----------------------------------PCA9685 ---------------------
 ...
 Wire.begin();                       // Wire must be started first
  ...
  for(int i=0;i < ios; i++){
     pwmController.setChannelPWM(i, 0 << 3);  // set up start values for Diverter --off
                                                                                
}
  
    for(int i=ios;i < (ios+GTIios); i++){       // setup start up value for GTI -- on
     pwmController.setChannelPWM(i, 512 << 3);
...
//---------------------------start bubble search for GTI contol------
if (grid != 0 ) {  
if (invert >=0) {
...
step2 = (prestep1 + step1);  // just  to stop the positive values creation
  ...
  curgrid2 = (512 * curinvt  ); // calculates what step
...
curelem3 = (curelem3 - stepbu3);
  ...
  curgrid3 = ( 512 * curelem3  );
  curgrid3 =(0-curgrid3);  
  if (stepbu3 < 0) {curgrid3= 512;}
...
if (grid !=0) {
  ...
  //curgrid = 0;
  stepc = (grid / element); // uses a fixed element size  to calculat step
...
if (stat != statb) {
  ...
  for(int i=statc;i <ios; i++){
     pwmController.setChannelPWM(i, 0 << 3);
     ...
     pwmController.setChannelPWM(stat, curgrid << 3);
  }
  ...
  if (type == 1){
   // Serial.println(" solar Diversion -  In Unison");
    for(int i=0;i < ios; i++){
     pwmController.setChannelPWM(i, curgrid << 3);
...
//--------------------------------GTIlimiter---------------
curgrid2= (512-curgrid2); // to inverts the value
if (curgrid2==512){curgrid2=511;}
if (full == 1){
stat2 = (0-stepbv);
if (GTI == 1) {
...
if (stat2 != statb2) {
  ...
  pwmController.setChannelPWM((stat2+ios), curgrid2 << 3);
  ...
  //  Serial.println(" GTI Limiting - Unison");
    for(int i=GTIios;i < (ios+GTIios); i++){
     pwmController.setChannelPWM(i, curgrid2 << 3);
...
if (GRID == 1){
 ...
 // Serial.println(" Battery to GTI ");
  ...
  pwmController.setChannelPWM(15, curgrid3 << 3);
 // Serial.print("15");
  //  Serial.print(" ");
//Serial.println(curgrid3);
  //
